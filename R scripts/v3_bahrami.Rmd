...
```{r} 
accuracies_raw <- read_csv("C:/Users/bruna/Desktop/GitHub/coupled-esns/data/v3/accuracies_N=20_bxbr_final.csv")
```
...
acc_ratio$measure <- factor(acc_ratio$measure, levels = c("d_integr", "d_intera"), labels = c("integration w/ self-feedback", "integration w/ pooled-feedback"))
```
...
for(n in (c(0.5, 1.0, 2.0))){
  ggplot(acc_ratio %>% filter(noise_1 %in% c(0.0, n), noise_2 %in% c(0.0, n)),
         aes(x=min_max, y=dyad_max, shape=noise)) +
    facet_wrap(~measure, ncol=2) +
    annotate("rect", xmin = 0, xmax = 1, ymin = 1, ymax = 2, alpha = 0.2, fill = "darkgreen") +
    annotate("rect", xmin = 0, xmax = 1, ymin = 0, ymax = 1, alpha = 0.2, fill = "darkred") +
    geom_point(alpha=1, size=3, stroke=2) +
  ...
  ggsave(filename = paste0("v3_ratios_nsigma=", n, "_bxbr.png"), bg = "white", width=15, height=7)
}
```

```{r}
acc_ratio_ambient <- acc_ratio %>%
  filter(noise_1 != 0.0, noise_2 != 0.0) %>%
  mutate(noise = case_when(noise_1==0.5 & noise_2==0.5 ~ "no noise", T ~ noise),
         noise = factor(noise,levels = c("no noise", "unequal noise", "equal noise")))
  

# cols <- sequential_hcl(5, palette = "batlow")[3:1]
# cols <- diverging_hcl(5, palette = "berlin", l1=75, p2=5)[c(1,5)]

for(n in (c(1.0, 2.0))){
  ggplot(acc_ratio_ambient %>% filter(noise_1 %in% c(0.5, n), noise_2 %in% c(0.5, n)), 
         aes(x=min_max, y=dyad_max, shape=noise)) +
    facet_wrap(~measure, ncol=2) +
    annotate("rect", xmin = 0, xmax = 1, ymin = 1, ymax = 2, alpha = 0.2, fill = "darkgreen") +
    annotate("rect", xmin = 0, xmax = 1, ymin = 0, ymax = 1, alpha = 0.2, fill = "darkred") +
    ...
    geom_smooth(method = "lm", aes(group=1), color = "black", se = F) +
    # geom_hline(yintercept=1, linetype="dashed") +
    geom_segment(aes(x=0, xend=1, y=1, yend=1), linetype="dashed", size=1) +
    # scale_color_manual(values = cols) + 
    scale_shape_manual(values = c("no noise" = 0, "unequal noise" = 1, "equal noise" = 2)) +
    xlim(0,1) +
    ylim(0, 2.1) +
    labs(x = expression(italic(P[min] / P[max])), y = expression(italic(P[dyad] / P[max]))) +
    theme_minimal() +
    theme(legend.title = element_blank(),
  ...
  ggsave(filename = paste0("v3_ratios_nsigma=", n, "_bxbr_ambient.png"), bg = "white", width=15, height=7)
...
acc_ratio_summary <- acc_ratio %>%
  filter(noise_1 %in% c(0.5, 2), noise_2 %in% c(0.5, 2)) %>%
  mutate(noise = case_when(noise_1==noise_2 & noise_1==0.5 ~ "no noise", T ~ noise)) %>%
