...
preds$cond <- factor(preds$cond, levels = c("auto", "allo", "poly"))
```
...
ggplot(traj_plot %>% filter(cond != "allo"), aes(x=timestep, y=m, color=label, shape=label)) +
  facet_wrap(~cond, labeller = labeller(cond = c("auto" = "d_integr", "poly" = "d_intera"))) +
  geom_line(color="black") +
  geom_ribbon(aes(ymin=m-se, ymax=m+se, fill=label), alpha=0.5) +
...
ggsave(filename = "v3_rotraj_800_01_02/overall_rotraj.png")
```
