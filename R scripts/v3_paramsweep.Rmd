...
psweep_df$rsize  <- as.numeric(as.character(psweep_df$rsize))
psweep_df$tsigma <- as.numeric(as.character(psweep_df$tsigma))
psweep_df$facet_label <- "model space,\nself-feedback"

x_breaks <- c(50, 100, 200, 400, 800, 1600)
y_breaks <- c(0.2, 0.4, 0.8, 1.6, 3.2, 6.4)

p1 <- ggplot(psweep_df, aes(x = rsize, y = tsigma, fill = m)) +
  facet_wrap(~facet_label) +
  geom_tile(color="white") +
  scale_x_log10(breaks = x_breaks, labels = x_breaks) +
  scale_y_log10(breaks = y_breaks, labels = y_breaks) +
  scale_fill_viridis_c(name = expression(italic(P[esn])),
                       guide = guide_colorbar(direction="horizontal",
                                              barwidth=unit(12,"cm"),
                                              barheight=unit(0.8,"cm"),
                                              title.position="top",
                                              title.hjust=0.5)) +
  labs(x = expression(italic(N[res])), y = expression(italic(sigma[teacher]))) +
  theme_minimal() +
  theme(axis.text = element_text(size=48),
        ...
        legend.title = element_text(size=50, margin = margin(r = 20)),
        legend.position = "top",
        legend.box = "horizontal",
        panel.grid.minor = element_blank(),
        axis.title.x = element_blank()) 

# ggsave(filename = "manuscript_figs/pweep.png", bg = "white", width = 9, height = 7)
```
...
diff_max <- sweep_cleaned %>%
  ...
  mutate(d_integr = round(d_integr-max, 2),
         d_intera = round(d_intera-max, 2)) %>%
  select(plink, tsigma, rsize, d_integr, d_intera) %>%
  pivot_longer(cols = c(d_integr, d_intera), names_to = "measure", values_to = "diff") %>%
  filter(plink=="0.1")

diff_max$txt_col <- ifelse(diff_max$diff >= 0.1, "black", "white")
diff_max$measure <- factor(diff_max$measure, levels = c("d_integr", "d_intera"))
diff_max$rsize  <- as.numeric(as.character(diff_max$rsize))
diff_max$tsigma <- as.numeric(as.character(diff_max$tsigma))

x_breaks <- c(50, 100, 200, 400, 800, 1600)
y_breaks <- c(0.2, 0.4, 0.8, 1.6, 3.2, 6.4)

p2 <- ggplot(diff_max, aes(x = rsize, y = tsigma, fill = diff)) +
  facet_wrap(~measure,
             labeller = labeller(measure = c("d_integr" = "no coupling,\nself-feedback",
                                             "d_intera" = "coupling via\npooled-feedback"))) +
  geom_tile(color="white") +
  # geom_text(aes(label = round(diff, 2)), color = diff_max$txt_col, size = 2) +
  scale_x_log10(breaks = x_breaks, labels = x_breaks) +
  scale_y_log10(breaks = y_breaks, labels = y_breaks) +
  scale_fill_continuous_diverging(palette="berlin",
                                  limits=c(-0.1, 0.1),
                                  ...
                                  guide = guide_colorbar(direction="horizontal",
                                              ...
                                              title.position="top",
                                              title.hjust=0.5)) +
  labs(x = expression(italic(N[res])),
       y = expression(italic(sigma[teacher])),
       fill = expression(italic(P[dyad] - P[max]))) +
  theme_minimal() +
  theme(axis.text = element_text(size=48),
        axis.text.x = element_text(angle = 45, hjust=1),
        axis.title = element_text(size=50),
        strip.text = element_text(size=48),
        legend.text = element_text(size=48),
        legend.title = element_text(size=50, margin = margin(r = 40)),
        legend.position = "top",
        legend.box = "horizontal",
        panel.grid.minor = element_blank(),
...
# ggsave(filename = "manuscript_figs/pweep_w_diff.png", bg = "white", width = 21, height = 12)
```
...
psweep_df$rsize  <- as.numeric(as.character(psweep_df$rsize))
psweep_df$tsigma <- as.numeric(as.character(psweep_df$tsigma))
psweep_df$plink  <- as.numeric(as.character(psweep_df$plink))

x_breaks <- c(50, 100, 200, 400, 800, 1600)
y_breaks <- c(0.2, 0.4, 0.8, 1.6, 3.2, 6.4)

ggplot(psweep_df, aes(x = rsize, y = tsigma, z = m)) +
  facet_wrap(~plink, labeller = labeller(plink = c("0.1" = "plink=0.1", "0.3" = "plink=0.3", "0.5" = "plink=0.5"))) +
  geom_tile(aes(fill = m)) +
  geom_contour(color = "black", bins = 10, na.rm = TRUE) +
  scale_x_log10(breaks = x_breaks, labels = x_breaks) +
  scale_y_log10(breaks = y_breaks, labels = y_breaks) +
  scale_fill_viridis_c(name = "pcorrect") +
  theme_minimal() +
  theme(axis.text = element_text(size=26),
        ...
        legend.title = element_text(size=28, angle = -90, vjust = 3, hjust = 0.5)) +
  guides(fill = guide_colorbar(barwidth = 2, barheight = 25))

# ggsave(filename = "manuscript_figs/pweep_full.png", bg = "white", width = 21, height = 7)
```
