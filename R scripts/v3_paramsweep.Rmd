...
```{r}
base_path <- "C:/Users/bruna/Desktop/GitHub/coupled-esns/data/v3/"
```

```{r, message=F}
batch_1 <- read_csv(paste0(base_path, "psweep_same/psweep_batch_1.csv"))
batch_2 <- read_csv(paste0(base_path, "psweep_same/psweep_batch_2.csv"))
batch_3 <- read_csv(paste0(base_path, "psweep_same/psweep_batch_3.csv"))
batch_4 <- read_csv(paste0(base_path, "psweep_same/psweep_batch_4.csv"))
batch_5 <- read_csv(paste0(base_path, "psweep_same/psweep_batch_5.csv"))
batch_6 <- read_csv(paste0(base_path, "psweep_same/psweep_batch_6.csv"))
...
```{r}
sweep_cleaned$measure <- factor(sweep_cleaned$measure, levels = c("min", "avg", "max", "d_integr", "d_intera"))
...
ggsave(filename = "v3_psweep.png", bg = "white", width=9, height=7)
```
...
ggsave(filename = "v3_psweep_diffavgtotal.png", bg = "white", width=6, height=7)
```
...
ggsave(filename = "v3_psweep_diffmax.png", bg = "white", width=6, height=7)
```
...
diff_dd$rsize <- factor(diff_dd$rsize)
```
...
ggplot(diff_dd, aes(x=rsize, y=tsigma, fill=diff)) +
  facet_grid(plink~., labeller = labeller(plink = c("0.1" = "plink=0.1", "0.55" = "plink=0.55", "1" = "plink=1.0"))) +
  geom_tile(color="white") +
  scale_fill_continuous_diverging(palette="berlin",
                                  limits=c(-0.1, 0.1), oob = scales::squish, l1=75, p2=5, rev=T,
                                  guide = guide_colorbar(label=F, title.position="right", title.hjust = 0.5, barwidth = 1, barheight = 25)) +
...
ggsave(filename = "v3_psweep_diffdd.png", bg = "white", width=4, height=7)
```
