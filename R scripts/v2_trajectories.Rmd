...
```{r}
base_path <- "C:/Users/bruna/Desktop/GitHub/coupled-esns/data/v2/ro_800_01_04/"
```
...
```{r, message=FALSE}
conds <- c("auto", "allo", "poly_parall", "poly_integr")
...
preds$cond <- factor(preds$cond, levels = c("auto", "allo", "poly_parall", "poly_integr"))
...
for(targ in c(1:9)){
  ...
  ggsave(filename = paste0("rotraj_", targ, ".png"), width = 25, height = 10) 
}
```
...
for(targ in c(1:9)){
  ...
  ggsave(filename = paste0("dyad_rotraj_", targ, ".png"), width = 25, height = 4.5) 
}
```
...
ggsave(filename = paste0("avgcum_dyad_rotraj.png"), width = 25, height = 4.5) 
```
