...
def run_models(index):
    ...
    for (x, y) in zip(X_train, Y_train):
        ...
        # reset reservoirs
        reservoir1.reset() # reservoir1.reset(np.random.random(reservoir1.state().shape)) # reservoir1.reset()
        reservoir2.reset() # reservoir2.reset(np.random.random(reservoir2.state().shape)) # reservoir2.reset()
...
#     # print(' ')
    ...
    pred1 = [y1 for y1 in Y_pred1]
    pred1 = evaluate(predictions=[pred1], target=Y_test, endpoint=False)

    pred2 = [y2 for y2 in Y_pred2]
    pred2 = evaluate(predictions=[pred2], target=Y_test, endpoint=False)

    return R_states1, R_states2, Y_pred1, Y_pred2, joint_score, pred1, pred2
...
if __name__ == "__main__":
    ...
    for i in range(runs):
        ...
        preds1.append(results[i][5])
        preds2.append(results[i][6])
    ...
    print(f'ESN1 accuracy averaged over runs: {np.mean(preds1) * 100} %')
    print(f'ESN2 accuracy averaged over runs: {np.mean(preds2) * 100} %')
...
# 
#     Y1.to_csv(f'data/s23_randreset_100runs/Y1_cs={cs}_ns={ns}_r1={r1_nnodes}_r2={r2_nnodes}.csv')
#     Y2.to_csv(f'data/s23_randreset_100runs/Y2_cs={cs}_ns={ns}_r1={r1_nnodes}_r2={r2_nnodes}.csv')
#     R1.to_csv(f'data/s23_randreset_100runs/R1_cs={cs}_ns={ns}_r1={r1_nnodes}_r2={r2_nnodes}.csv')
#     R2.to_csv(f'data/s23_randreset_100runs/R2_cs={cs}_ns={ns}_r1={r1_nnodes}_r2={r2_nnodes}.csv')
# 
#     targets = pd.DataFrame(np.tile(np.concatenate(Y_test), (runs,1)))
#     targets.to_csv(f'data/s23_randreset_100runs/targets.csv')
# 
...
#     meta_data.to_csv(f'data/s23_randreset_100runs/meta_data.csv', index=False)
