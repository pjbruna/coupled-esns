...
for(i in c(0:4)){
  y1 <- read.csv(paste0("data/v2_s23_randreset_batch", i+1, "/Y1_ns=0.0_r1=500_r2=500.csv")) %>%
    setNames(c("n1", "n2")) %>%
    mutate(model = "1")
  
  y2 <- read.csv(paste0("data/v2_s23_randreset_batch", i+1, "/Y2_ns=0.0_r1=500_r2=500.csv")) %>%
    setNames(c("n1", "n2")) %>%
    mutate(model = "2")
...
if(mean(data$n1 == data$n2)==0){
  ...
  write.csv(performance_data, file="data/v2_s23_randreset_500_performance.csv")
} else {
